...
my @keywords = qw(my our local state elsif else given default continue format sub package use no require undef switch case print printf say grep map sort do eval goto redo next last return y tr qr qw qq qx q m s split keys values each);
...
my %implicitusers;
@implicitusers{@implicitusers} = @implicitusers;
delete @implicitusers{@keywords};

my %unary;
@unary{@unary} = @unary;
delete @unary{@implicitusers,@keywords};
...
delete @allknown{keys %unary,@acceptors,@operators,@argumentless,@moose,@extkeywords,@keywords, keys %implicitusers};

delete @implicitusers{keys %allknown};
...
say "IMPLICIT_USERS = ".join '|',  map "\"$_\"", reverse sort keys %implicitusers;
