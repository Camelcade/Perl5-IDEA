...
public class PerlBrewHandler extends PerlVersionManagerHandler<PerlBrewData, PerlBrewHandler> {
  static final String LIB_SEPARATOR = "@";
  private static final String PERLBREW = "perlbrew";
  ...
  public void createSdkInteractively(@NotNull PerlHostHandler<?, ?> hostHandler, @Nullable Runnable successCallback) {
    hostHandler.chooseFileInteractively(
      PerlBundle.message("perl.vm.perlbrew.choose.perlbrew"),
      it -> it.findFile(PERLBREW),
      it -> StringUtil.equals(it, PERLBREW),
      it -> {
        String fileName = Paths.get(it).getFileName().toString();
        return StringUtil.equals(fileName, PERLBREW) ? null : PerlBundle.message("perl.vm.perlbrew.wrong.file", fileName);
      },
  ...
  private void createSdkInteractively(@Nullable String selectedPath,
    ...
    PerlBrewAdapter perlBrewAdapter = new PerlBrewAdapter(selectedPath, perlHostData);
    List<String> distributions = perlBrewAdapter.getList();
    ...
    List<String> perlPath = perlBrewAdapter.execWith(installation, "perl", PERL_LE, PERL_CTRL_X);
    if (perlPath == null || perlPath.size() != 1) {
      return;
    }

    PerlBrewData perlBrewData = createData(selectedPath, installation);
  ...
  public String getMenuItemTitle() {
    return PerlBundle.message("perl.vm.perlbrew.menu.title");
  }
  ...
  private PerlBrewData createData(@NotNull String versionManagerPath, @NotNull String distributionId) {
