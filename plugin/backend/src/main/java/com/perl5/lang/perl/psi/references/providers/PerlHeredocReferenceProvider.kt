...
class PerlHeredocReferenceProvider : PsiReferenceProvider() {
  override fun getReferencesByElement(element: PsiElement, context: ProcessingContext): Array<out PsiReference> =
    (element as? PerlHeredocTerminatorElement)?.let { arrayOf(PerlHeredocReference(element)) } ?: PsiReference.EMPTY_ARRAY
}