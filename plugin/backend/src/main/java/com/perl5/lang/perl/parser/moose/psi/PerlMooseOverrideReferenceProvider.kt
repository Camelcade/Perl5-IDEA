...
class PerlMooseOverrideReferenceProvider: PsiReferenceProvider() {
  override fun getReferencesByElement(
    element: PsiElement,
    context: ProcessingContext
  ): Array<out PsiReference> {
    val provider = PsiTreeUtil.getParentOfType(
      element, PerlMooseOverrideStatement::class.java, true, PsiPerlStatement::class.java
    ) ?: return PsiReference.EMPTY_ARRAY

    var expr:  PsiElement? = provider.expr ?: return PsiReference.EMPTY_ARRAY
    ...
    if (expr is PsiPerlCommaSequenceExpr) {
      val lastElement: PsiElement? = expr.getLastChild()
