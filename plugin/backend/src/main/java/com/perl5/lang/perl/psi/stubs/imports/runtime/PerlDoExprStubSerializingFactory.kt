...
open class PerlDoExprStubSerializingFactory(elementType: IElementType) :
  ...
  ): PerlRuntimeImportStub {
    val importPath = StringUtil.nullize(PerlStubSerializationUtil.readString(dataStream))
    return PerlRuntimeImportStubImpl(parentStub, elementType, importPath)
  }

  override fun indexStub(stub: PerlRuntimeImportStub, sink: IndexSink): Unit = Unit
}