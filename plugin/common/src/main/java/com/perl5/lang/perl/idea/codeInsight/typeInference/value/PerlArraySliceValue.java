...
public class PerlArraySliceValue extends PerlParametrizedOperationValue {
  ...
  PerlArraySliceValue(@NotNull PerlValue arrayValue,
    ...
    if (arrayValue.isDeterministic() && indexesValue.isDeterministic()) {
      PerlValue.LOG.error("Bot array and indexes are deterministic and should be computed in-place: " +
                          "array=" + arrayValue + "; " +
  ...
  private static PerlValue computeResolve(@NotNull PerlValue arrayValue,
    ...
    if (indexValue instanceof PerlArrayValue perlArrayValue) {
      perlArrayValue.forEach(key -> builder.addElement(((PerlArrayValue)arrayValue).get(key)));
    }
    else {
      return null;
    }
