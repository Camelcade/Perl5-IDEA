...
%state PACKAGE_OR_VERSION, VERSION_OR_OPERAND, PACKAGE_OR_VERSION_OR_OPERAND, SUB_DECLARATION, BRACED_STRING, ATTRIBUTES
...
%state HANDLE_OR_OPERAND_OR_NOTHING, HANDLE_WITH_ANGLE
%state HANDLE_OR_OPERAND
...
<HANDLE_OR_OPERAND>{
	"@" 	{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_ARRAY);}
	"$#" 	{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_SCALAR_INDEX);}
	"$" 	{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_SCALAR); }
	"%"		{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_HASH);}
	"*"		{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_GLOB);}
	"&"		{return startUnbracedVariable(OPERATOR_OR_OPERAND, SIGIL_CODE);}
	"("		{return LEFT_PAREN;}
}
...
<YYINITIAL,AFTER_COMMA,OPERATOR,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,POSTFIX_OR_OPERATOR,SUB_DECLARATION,HANDLE_OR_OPERAND>{
	"{"     {yybegin(YYINITIAL);return getLeftBrace();}
}

// common logic
<YYINITIAL,AFTER_COMMA,OPERATOR,NESTED_ELEMENT,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,POSTFIX_OR_OPERATOR,HANDLE_OR_OPERAND>{
...
<YYINITIAL,AFTER_COMMA,OPERATOR,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,POSTFIX_OR_OPERATOR,HANDLE_OR_OPERAND>{
...
<YYINITIAL,AFTER_COMMA,NESTED_ELEMENT,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,HANDLE_OR_OPERAND_OR_NOTHING>{
...
<YYINITIAL,AFTER_COMMA,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,HANDLE_OR_OPERAND>{
...
<YYINITIAL,AFTER_COMMA,PACKAGE_OR_VERSION_OR_OPERAND,REGEX_OR_OPERAND,HANDLE_OR_OPERAND>{
	{PERL_VERSION}  		{yybegin(OPERATOR);return NUMBER_VERSION;}
}


<YYINITIAL,AFTER_COMMA,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,HANDLE_OR_OPERAND,HANDLE_OR_OPERAND_OR_NOTHING>{
	...
	{BARE_HANDLE_ACCEPTORS}				{yybegin(HANDLE_OR_OPERAND);return IDENTIFIER;}
	{NAMED_UNARY_BARE_HANDLE_ACCEPTORS}	{yybegin(HANDLE_OR_OPERAND);return OPERATOR_NAMED_UNARY;}
	...
	{PERL_OPERATORS_FILETEST} / [^a-zA-Z0-9_] 	{yybegin(HANDLE_OR_OPERAND_OR_NOTHING);return OPERATOR_FILETEST;}
	...
	{CORE_PREFIX}"use"	 		{yybegin(PACKAGE_OR_VERSION); return  RESERVED_USE;}
...
//	"finally"					{yybegin(YYINITIAL);return RESERVED_FINALLY;}
	...
	// special treatment?
	{CORE_PREFIX}"print"	 	{yybegin(HANDLE_OR_OPERAND); return RESERVED_PRINT;}
	{CORE_PREFIX}"printf"	 	{yybegin(HANDLE_OR_OPERAND); return RESERVED_PRINTF;}
	{CORE_PREFIX}"say"	 		{yybegin(HANDLE_OR_OPERAND); return RESERVED_SAY;}
...
<HANDLE_OR_OPERAND_OR_NOTHING>{
	{IDENTIFIER} / [:(-] {pullback(0);yybegin(YYINITIAL);}
	{IDENTIFIER} 	{yybegin(OPERATOR);return HANDLE;}
...
<HANDLE_OR_OPERAND>{
	{HANDLES} 	{yybegin(YYINITIAL);return HANDLE;}
}

<YYINITIAL,AFTER_COMMA,OPERATOR_OR_OPERAND,REGEX_OR_OPERAND,HANDLE_OR_OPERAND>{
	{QUALIFIED_IDENTIFIER} 	{yybegin(OPERATOR_OR_OPERAND);return getIdentifierToken();}
}
